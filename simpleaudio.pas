...
uses  Classes, SysUtils, Platform, HeapManager, Threads, GlobalConst;
...
var gpfsel4:cardinal     absolute _gpfsel4;      // GPIO Function Select 4
    ...
    SampleBuffer:cardinal absolute SampleBuffer_ptr;
...
threadsleep(1);
  ...
  repeat sleep(0) until (dma_cs and 2) <>0 ;
    begin
    // now we should call audiocallback to feel the buffer in desired sample format
    CurrentAudioSpec.callback(CurrentAudioSpec.userdata, @samplebuffer, CurrentAudioSpec.size);
    // the buffer has to be converted to 32bit integer
    ...
    if dma_nextcb=nocache+ctrl1 then noiseshaper(samplebuffer,dmabuf1,CurrentAudioSpec.oversample,CurrentAudioSpec.oversampled_size)
    else noiseshaper(samplebuffer,dmabuf2,CurrentAudioSpec.oversample,CurrentAudioSpec.oversampled_size);
    dma_cs:=3;
    end;
  until terminated;
end;
